...
## 移植注记（v0.20.0 统一原生侧栏基线 · feat/terminal-blocks-inputbox）

> 本设计最初在旧框架（unify 前）的 feat/terminal-blocks 分支分两版实现：
> 920549f（选中 popup）与 d976952（hover block 层 + 药丸），未随
> 2026-09-12「统一新框架」迁移。本分支在 v0.20.0 基线上按需求移植 **hover block 层**：
> 每块 CLI 悬停时浮出「添加到对话」药丸，点击把该块（命令 + 输出）整块插入
> **composer input-box**——复用 TextEditor 选区 popup 的链路
> （`appendToDraft` → `conversation.input.setDraft`，光标处拼接，见
> src/client/conversation-draft.ts）。

### 与旧实现的差异

- TerminalView props 新增 `ctx: Context`（builtins/tabs.tsx descriptor 透传），
  药丸提交 `appendToDraft(ctx, scope.sessionId, buildTerminalInsert(command, output))`。
- 未移植「选中文本 → popup」形态（本次需求仅 hover 药丸；区块内选中时药丸隐藏，
  与 v2 行为一致）；`blockForSelection` 保留（overlay hover 归属解析用）。
- `.terminalBlockPill` 自带完整 chrome（旧版仅写定位覆盖层，依赖 selectionPopup
  组合；新版把 chrome 收进自身，全部 `--dsw-alias-*` 令牌）。
- `.terminal` 补 `position: relative` 作为 block 层包含块。
- 懒加载不变：terminal-blocks.ts / TerminalBlockOverlay.tsx 随 terminal chunk
  （lib/client-terminal.js）下发，不进核心 bundle。
