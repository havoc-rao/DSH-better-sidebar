...
- **布局回归：host 根 div 撑高 body（18px 滚动条）**：右键菜单 `<Menu anchor={<span />}>` 是第一个挂在 Sidebar 根级（`[data-dsh-better-sidebar]` host 下、面板之外）的 Menu——ui-primitives 的 Menu 总会渲染一个 `position: relative` 的 anchor wrapper span，空 inline span 的行框 = 继承 line-height ≈18px → host div 高度 18px → body 出现滚动条、左栏/main 被顶出（F12 可见 host div 最下方多出一层高度）。FileTree/TabBar 的 Menu 都在 fixed 面板内部，从未暴露。修复：host div 改 `position: fixed; inset: 0; pointer-events: none`（零文档流占位、点击穿透），`.toggleCluster` / `.panel` / `.bottomPanel` 显式 `pointer-events: auto` 恢复交互；`layout-repro.sh`（真实 dsh web + Playwright 测量 body 滚动）验证各场景 `bodyScrollable: false`。
- **merge 回归：fixed host 层叠上下文隔离面板（盖不住宿主 UI）**：host 改 `position: fixed; inset: 0` 后自身形成 stacking context——面板的 z-40 被关在 host 上下文内，host 以 z-auto(0) 参与根层叠，低于宿主 composer（z-1）→ 底部/右面板盖不住对话框与输入区（实测：面板 z-1000 !important 仍命中宿主 input，host z-1000 才恢复）。修复：host 显式 `z-index: 40`（= 面板层）——host 上下文整体参与根层叠 40 层，内部子级排序（面板 40 / 簇 45 / 边界 50）不变，宿主浮层 100+ 仍盖住面板（语义与 host 普通流时代一致）。
