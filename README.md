...
- **🌿 Git 面板**：真 diff + VSCode 式 diff tab、历史、右键暂存 / 提交 / 还原；**AI 提交信息生成**——一键把已暂存变更（`--stat` + 完整 diff + 最近提交风格参考）交给 LLM 按所选模板起草提交信息（「侧边卡片」→ 源代码管理齿轮弹窗选择 LLM 提供方 / 模型 / 模板）；工作区容器下自动发现子仓库并显示**仓库选择器**，支持 linked worktree 变更发现
...
- ✨ **AI 提交信息生成**（Git 面板提交框）：一键把**已暂存变更**格式化打包（`--stat` 概览 + 完整 diff（24KB 封顶）+ 最近 N 条提交信息作为**风格参考**）交给 LLM 起草提交信息——引擎走 DSH 自带 `ctx.llm` 服务（无额外 key 配置），在「侧边卡片 → 源代码管理 → 齿轮弹窗」自定义面板中选择 **LLM 提供方 / 模型 / 提交参考模板**（约定式提交 / Gitmoji / 简洁直白 / 自定义指令）与历史风格参考条数；生成结果填入提交框可直接编辑提交，语言自动跟随仓库历史（中文仓库出中文）
