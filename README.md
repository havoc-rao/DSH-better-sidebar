...
| ⌘⌥⇧B | 进入 / 退出 IDE 模式：右侧面板占满全屏（类 VSCode 独立窗口，进入时自动打开面板） | 全局（+ 菜单打开时除外） |
| ⌘F | 聚焦文件搜索框 | 当前标签是文件窗口 |
...
- ⌨️ **视图切换快捷键**（VSCode 心智）：`⌘⇧E` 显示/收起资源管理器（vscode 布局开关 Side Bar 抽屉、docked 布局打开文件窗口、已打开时再按关闭——与 Activity Bar 图标同款开关语义）、`⌘⇧G` 打开源代码管理（Git 面板）、**`⌘⌥⇧B` 进入/退出 IDE 模式**——右侧面板占满全屏（宽 100vw、z-index 1000 盖住应用壳、释放布局挤压、底部面板被盖在身后，类 VSCode 独立窗口；进入自动开面板，面板右上角 ✕ 或再按同键退出，恢复停靠宽度）——全局生效，+ 菜单打开时让行
