...
- **🗂️ 文件工作台**：资源管理器（懒加载目录树；软链接按目标类型展示——目录软链接可展开、失效链接标红；**VSCode 式 git 状态装饰**——文件/目录**名称按状态着色**（修改琥珀 / 新增绿 / 删除红划线 / 未跟踪灰）+ 行尾彩色字母徽标 M/A/D/R/C/U/!/T、目录聚合子孙状态、底部变更计数条，随 Git 面板操作实时联动）+ CodeMirror 编辑器；图片 / Markdown（含 Mermaid 图表，strict 安全渲染 + 点击放大；README 级内嵌 HTML——徽章墙 / `<details>` 折叠 / 表格内联标签经 DOMPurify 消毒真实渲染；浮动目录大纲一键跳转）/ HTML / PDF / Office 内联预览
...
- **🌿 Git 面板**：真 diff + VSCode 式 diff tab、历史、右键暂存 / 提交 / 还原；**AI 提交信息生成**——一键由 DSH agent-loop 的专用单轮 agent 处理已暂存变更（`--stat` + 完整 diff + 最近提交风格参考），暂存区为空时改为压缩当前 `git status` 与工作区 diff 后给出候选总结（在「侧边卡片」设置顶部的「Git 提交 Agent」中选择 LLM 提供方 / 模型 / 模板）；工作区容器下自动发现子仓库并显示**仓库选择器**，支持 linked worktree 变更发现
...
- ➕ **+ 菜单键盘操作**：打开后直接按 `1-9`（位置）或选项首字母选择，↑↓ / Home / End 移动高亮，Enter 确认，Esc 关闭——每行名称右侧显示一个「数字/字母」chip（如 `4/T`），菜单底部另有操作提示行
...
| ⌘⇧E | 显示 / 收起资源管理器（vscode 布局开关 Side Bar 抽屉；docked 布局打开文件窗口，已打开时再按关闭它） | 全局（+ 菜单打开时除外） |
| ⌘⇧G | 打开源代码管理（Git 面板） | 全局（+ 菜单打开时除外） |
| ⌘⌥⇧B | 进入 / 退出 IDE 模式：右侧面板占满全屏（类 VSCode 独立窗口，进入时自动打开面板；资源管理器钉左缘、侧边对话钉成右缘聊天列、底部面板停靠到文件 tabs 下方） | 全局（+ 菜单打开时除外） |
| ⌘F | 聚焦文件搜索框 | 当前标签是文件窗口 |
| ⌘Tab / ⌘Shift+Tab | 下一个 / 上一个标签 | 焦点在侧边栏内 |
| ⌘1…⌘9 | 跳到当前窗格第 n 个标签 | 焦点在侧边栏内 |
| ⌘W | 关闭当前标签 | 焦点在侧边栏内 |

**+ 菜单**（打开后直接按）：`1-9` 按位置选择 · 字母键按选项选择（字母取自选项 id，如 `terminal`→T、`git`→G，中文标签同样可用；重复按同字母继续循环） · `↑↓` / `Home` / `End` 移动高亮 · `Enter` 确认 · `Esc` 关闭——每行名称右侧显示一个「数字/字母」chip（如 `4/T`）。
...
- ✨ **AI 提交信息生成**（Git 面板提交框）：专用的**单轮、无工具 Git agent** 通过 DSH `ctx.agents.create()` / agent-loop 运行；优先把**已暂存变更**格式化打包（`--stat` 概览 + 完整 diff（24KB 封顶）+ 最近 N 条提交信息作为**风格参考**），暂存区为空时则压缩 `git status`（8KB 封顶）与 tracked working-tree diff，保留未跟踪路径并生成保守的候选总结。在「侧边卡片」设置顶部的「Git 提交 Agent」中选择 **LLM 提供方 / 模型 / 提交参考模板**；结果填入提交框供编辑，不会自动暂存或提交

- 🧱 **VSCode 风格工作台布局**：新 `sidebarLayout` 设置（默认 `docked`，编辑器卡齿轮下拉）——`vscode` 模式把文件树从编辑器 tab 内搬到**独立资源管理器抽屉**（编辑器左 | 抽屉中 | 活动栏右的镜像排列，编辑器贴近聊天区），**抽屉由 Activity Bar 的资源管理器图标展开/收起**（收起时编辑器占满，展开时图标高亮），并在面板右缘加 **Activity Bar**（48px 图标化 `+` 菜单：资源管理器图标 = 抽屉开关，其余图标打开对应 tab，活动 tab 类型带内缘指示条；与 `+` 菜单同一套注册表/排序/禁用判定）；面板顶部一条 **tab strip header**（活动 pane 的 tabs，右上角折叠按钮悬浮其上；编辑器 pane 内不再渲染 tab 条，点击另一 pane 其 tabs 进入 header）；编辑器 tab 在 vscode 模式下去掉内嵌树与树开关，文件经抽屉树打开为 per-path 标签；**`sideBarSide` 可整体镜像布局**（默认右 = `编辑器 | 文件树列 | 活动栏`，编辑器贴近聊天区；切到左 = `活动栏 | 文件树列 | 编辑器`——**图标列与文件树列一起翻到左缘**，边框/活动指示条/拖拽手柄/图标 tooltip 方向全部自动镜像），**切换按钮钉在 Activity Bar 图标列底部**（VSCode 活动栏设置齿轮的位置，一点即翻转、实时生效并持久化）；窄视口自动回退 docked 抽屉；legacy `vscode-left`/`vscode-right` 值自动迁移
- 🎨 **资源管理器 Git 状态装饰**（VSCode 契约）：文件/目录**名称按状态着色**（主信号——修改琥珀、增/重命名绿、删除红、未跟踪灰），行尾再缀彩色字母徽标——M 已修改 / A 已添加 / D 已删除 / R 重命名 / C 复制 / U 未跟踪 / ! 冲突 / T 类型变更，颜色映射 DSH 语义令牌（修改→warn、增/重命名→success、删/冲突→error、未跟踪→灰）；**目录行聚合子孙状态**的最高优先级（冲突 > 忽略 > 修改/复制/类型变更 > 新增/删除/重命名/未跟踪，删除不向父目录传播——VSCode `propagate:false` 语义）；**已删除文件名称划线**（红色由状态色承担）；面板底部一条**变更计数条**（按严重度排序，如 `M1 · D1 · U3`）；状态随 Git 面板的刷新/暂存/提交/放弃**实时联动**（共享变更总线，无需文件监听），会话 cwd 位于仓库子目录时只装饰可见范围（[设计文档](docs/plans/2026-08-19-git-status-decorations-design.md)）
- ⌨️ **视图切换快捷键**（VSCode 心智）：`⌘⇧E` 显示/收起资源管理器（vscode 布局开关 Side Bar 抽屉、docked 布局打开文件窗口、已打开时再按关闭——与 Activity Bar 图标同款开关语义）、`⌘⇧G` 打开源代码管理（Git 面板）、**`⌘⌥⇧B` 进入/退出 IDE 模式**——右侧面板占满全屏（宽 100vw、z-index 1000 盖住应用壳、释放布局挤压、底部面板被盖在身后，类 VSCode 独立窗口；进入自动开面板并默认展开 explorer 抽屉，面板右上角 ✕ 或再按同键退出，恢复停靠宽度）——全局生效，+ 菜单打开时让行。**IDE 模式（v0.18.0）**：全屏盖住主会话后，**侧边对话（Side Chat）可钉成右缘聊天列**（Cursor 风格，**默认收起**——进入模式不再自动弹出）——镜像当前 pane 激活的 sidechat tab（无 tab 时显示「新建对话」hero），列头收起钮 / **面板右上角 ✕ 旁的开合钮** / **Activity Bar 的 Side Chat 图标**开合，**左缘拖拽调宽**（240~480px 默认 360，随会话持久化）；聊天 tab 不再占用编辑器工作台，底部面板右缘收在聊天列左缘
