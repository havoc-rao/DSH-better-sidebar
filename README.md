...
- **🗂️ 文件工作台**：资源管理器（懒加载目录树；软链接按目标类型展示——目录软链接可展开、失效链接标红；**VSCode 式 git 状态装饰**——文件/目录**名称按状态着色**（修改琥珀 / 新增绿 / 删除红划线 / 未跟踪灰）+ 行尾彩色字母徽标 M/A/D/R/C/U/!/T、目录聚合子孙状态、底部变更计数条，随 Git 面板操作实时联动）+ CodeMirror 编辑器；图片 / Markdown（含 Mermaid 图表，strict 安全渲染 + 点击放大）/ HTML / PDF / Office 内联预览
...
- **🌿 Git 面板**：真 diff + VSCode 式 diff tab、历史、右键暂存 / 提交 / 还原；**AI 提交信息生成**——一键把已暂存变更（`--stat` + 完整 diff + 最近提交风格参考）交给 LLM 按所选模板起草提交信息（「侧边卡片」→ 源代码管理齿轮弹窗选择 LLM 提供方 / 模型 / 模板）
