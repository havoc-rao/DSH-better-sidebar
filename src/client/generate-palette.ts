/**
 * Extended 256-color palette generation (indices 16–255) for the terminal.
 *
 * Ported from tabby-terminal's generatePalette.ts (MIT License,
 * Copyright (c) 2017 Tabby Developers): the 16 curated ANSI colors plus the
 * surface background/foreground are interpolated in LAB color space into a
 * harmonious full-range palette — the 6×6×6 color cube (16–231) and the
 * 24-step grayscale ramp (232–255). Light themes invert the interpolation
 * corners so the ramp runs fg→bg instead of bg→fg (`harmonious` disables
 * that inversion for color schemes that want a literal dark ramp).
 *
 * Kept as a pure module (no DOM, no xterm) so it is unit-testable without
 * mounting a terminal; the result feeds `ITheme.extendedAnsi` in
 * TerminalView. User-defined entries beyond index 15 in the input `colors`
 * array are preserved verbatim and never overwritten.
 */
...
/**
 * Generate the extended 256-color palette (indices 16–255) from the base 16
 * colors using LAB color space interpolation. User-defined colors beyond
 * index 15 in `colors` are preserved and not overwritten.
 *
 * @param colors - Array of hex color strings (at least 16; entries beyond 16
 *   are user-defined extended colors).
 * @param bg - Background color as hex string.
 * @param fg - Foreground color as hex string.
 * @param harmonious - If true, disable the light-theme corner inversion.
