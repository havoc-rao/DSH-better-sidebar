...
- **🌿 Git Panel**: real diff + VSCode-style diff tabs, history, right-click to stage / commit / revert; **AI commit-message drafting** — one click turns the staged changes (`--stat` + full diff + recent-commit style reference) into a draft through the LLM of your choice (Side Card settings → Source Control gear popup picks the LLM provider / model / template); under a workspace container it discovers child repositories and shows a **repository selector**, with linked-worktree change discovery
...
- ✨ **AI commit-message drafting** (Git panel commit box): one click packs the **staged changes** into an LLM-friendly bundle (`--stat` overview + full diff (capped at 24KB) + the most recent N commit subjects as a **style reference**) and drafts a commit message — the engine rides DSH's own `ctx.llm` service (no extra API-key wiring); the Side Card settings → Source Control → gear popup custom panel selects the **LLM provider / model / commit reference template** (Conventional Commits / Gitmoji / Plain / custom instructions) and the style-reference count; the draft lands in the editable message box, and the language follows the repository history (Chinese repos get Chinese messages)
