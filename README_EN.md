...
- **🗂️ File Workbench**: file explorer (lazy-loading tree; symlinks show their target kind — directory links expand, dangling links flagged; **VSCode-style git status decorations** — file/folder **names tinted by status** (modified amber / added green / deleted red + strikethrough / untracked gray) plus colored M/A/D/R/C/U/!/T letter badges on rows, folders aggregate descendant status, a change-count footer that stays in sync with Git panel actions) + CodeMirror editor; inline preview for images / Markdown (incl. Mermaid diagrams, strict-mode safe rendering + click-to-zoom) / HTML / PDF / Office
...
- **🌿 Git Panel**: real diff + VSCode-style diff tabs, history, right-click to stage / commit / revert; **AI commit-message drafting** — a dedicated one-turn DSH agent-loop worker drafts from staged changes (`--stat` + full diff + recent-commit style reference), or compresses `git status` and the working-tree diff into a tentative summary when the index is empty (pick the LLM provider / model / template in the top-level Git Commit agent section of Side Card settings); under a workspace container it discovers child repositories and shows a **repository selector**, with linked-worktree change discovery
...
- ✨ **AI commit-message drafting** (Git panel commit box): a dedicated **one-turn, tool-free Git agent** runs through DSH `ctx.agents.create()` / agent-loop. It prefers an LLM-friendly bundle of the **staged changes** (`--stat` overview + full diff capped at 24KB + recent commit subjects as a style reference); with an empty index it compresses `git status` (8KB cap) and the tracked working-tree diff, retains untracked paths, and produces a conservative tentative summary. Pick the **LLM provider / model / commit reference template** in the top-level Git Commit agent section of Side Card settings; the editable result never stages or commits automatically

- 🧱 **VSCode-style workbench layout**: a new `sidebarLayout` setting (default `docked`, under the editor card's gear) — `vscode` mode moves the file tree out of the editor tab into an **independent explorer drawer** (mirror arrangement: editor | drawer | activity bar, the editor stays by the chat) **toggled by the Activity Bar's explorer icon** (collapsed → the editor takes the full width; expanded → the icon highlights), plus a **48px Activity Bar** on the panel's right edge that iconizes the `+` menu (the explorer icon toggles the drawer; the rest open their tab types, the active tab's type gets an inner-edge indicator; same registry/order/`available` gating as the `+` menu). The panel's top is a **single tab-strip header** (the ACTIVE pane's tabs, with the corner collapse buttons floating on it; editor panes no longer render their own strips — clicking another pane promotes its tabs into the header). In vscode mode editor tabs drop their docked tree + toggle, files open as per-path tabs through the drawer's tree; **`sideBarSide` mirrors the WHOLE arrangement** (default right — `editor | side bar | activity bar`, the editor stays by the chat; flipped left — `activity bar | side bar | editor`: **the icon bar and the file-tree column move to the panel's left edge together**, borders / active indicator / resize handle / icon-tooltip sides all mirror automatically), and the **toggle button pins to the Activity Bar's bottom** (the VSCode activity-bar settings-gear spot — one tap flips it, persisted, effective immediately); narrow viewports fall back to the docked drawer automatically; legacy `vscode-left`/`vscode-right` values migrate on read
- 🎨 **Explorer git status decorations** (the VSCode contract): file/folder **names are tinted by status** (the primary signal — modified amber, added/renamed green, deleted red, untracked gray) with colored status-letter badges beside them — M modified / A added / D deleted / R renamed / C copied / U untracked / ! conflict / T type-changed, colors mapped onto DSH semantic tokens (modified→warn, added/renamed→success, deleted/conflict→error, untracked→muted); **folder rows aggregate the highest-priority descendant status** (conflict > ignored > modified/copied/type-changed > added/deleted/renamed/untracked, and deleted files do NOT propagate to parents — VSCode's `propagate:false` semantics); **deleted file names are struck through** (the red comes from the status color); a **change-count footer** at the panel bottom (most severe first, e.g. `M1 · D1 · U3`); the overlay **stays live with the Git panel** (stage/commit/discard recolor the tree through a shared change bus — no file watcher), and when the session cwd sits inside a repository subdirectory only the visible scope is decorated ([design](docs/plans/2026-08-19-git-status-decorations-design.md))
- ⌨️ **View-switch shortcuts** (VSCode muscle memory): `Cmd/Ctrl+Shift+E` shows/hides the Explorer (vscode layout: toggles the Side Bar drawer; docked: opens the Files window, pressing again closes it — the same toggle semantics as the Activity Bar icon), `Cmd/Ctrl+Shift+G` shows Source Control (the Git panel), **`Cmd/Ctrl+Alt+Shift+B` toggles IDE mode** — the right panel fills the whole screen (width 100vw, z-index 1000 over the app shell, layout push released, the bottom panel covered behind it — a VSCode-window-like state; entering opens the panel, the top-right ✕ or the same key exits back to the docked width) — global, yielding only while a + menu is open
...
| ⌘⇧E | Show / hide Explorer (vscode layout: toggles the Side Bar drawer; docked: opens the Files window, and closes it when it's already the one in view) | Always (yields while the + menu is open) |
| ⌘⇧G | Show Source Control (Git panel) | Always (yields while the + menu is open) |
| ⌘⌥⇧B | Toggle IDE mode: the right panel fills the whole screen (a VSCode-window-like state; entering opens the panel) | Always (yields while the + menu is open) |
