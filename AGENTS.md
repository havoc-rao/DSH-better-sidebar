...
interface TabComponentProps {
  ...
  projectDir?: string                       // 全局工作区项目根目录（新建终端的启动目录；'' = 未设定）
  onSetProjectDir?: (dir: string) => void   // 设定项目根目录（持久化）
}
```
