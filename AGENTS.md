...
> 标注 `focusInSidebar` 的键位在面板内交互时才生效（⌘B 打开面板后焦点尚在会话区，需点击面板内再按）——**有意为之**：避免在会话编辑区吞掉 Ctrl+Tab / Ctrl+W 等宿主键；`Cmd+P` / `Cmd+F` 在侧边栏外输入（composer / 终端 / 编辑框）时同样放行给页面。插件绑定若要避开 + 菜单打开状态，`when` 里判 `!context.plusMenuOpen`。**tab 条自 v0.17.0 起可聚焦**：TabBar 的 tab 元素带 `tabIndex={0}`，主键 mousedown 时 `focus()` 自身 + Enter/Space 键盘激活（等同点击）——「点击 tab」会把真实焦点带进宿主，否则纯鼠标用户在面板内按 ⌘W/⌘⇧W/⌥W 永远过不了 `focusInSidebar` 门（tab 是 div，无 tabindex 不可聚焦，`activeElement` 停留在 body）。**W 系关闭的目标是「焦点钉住的 tab」**（`workingTabIdOf`），**不是 state 的 `active` 高亮**——钉住有两个来源：① **内容焦点**（document `focusin` 追踪 + 三处 `data-dsh-tab-id` 属性——面板 tab 内容包裹层 / 自由窗口根 / tab 条）；② **激活事件**（service 的 `openTab`/`activateTab` 落位后即钉住——刚打开/刚激活的 tab 即使没点进内容区也是工作表面，类浏览器「新开的标签页就是当前页」语义；定向到非激活会话的打开/激活不钉）。在底部面板的终端里打字但 `activePane` 仍指着右侧面板时，⌘W/⌘⇧W/⌥W 关闭的是**底部那个 tab**；焦点不在任何 tab 表面、或钉住的 tab 已被关闭/属另一会话时，回退到 state-active。桌面 ⌘W 认领（`claimTargetOf`）与绑定共用同一 `workingTabIdOf`，两条路径关闭的永远是同一个 tab。
