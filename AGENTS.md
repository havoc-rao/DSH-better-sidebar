...
| `global` | 60 | 是 | 否 | 全局工作区页：记录所有**实例级全局状态**——目前是全局共享窗口（`gb:` 前缀的「所有项目同步」终端）清单；数据来自宿主注入的 `TabComponentProps.globalWindows`（workspace windows store 的实例级 blob），无则显示引导文案。两种形态：**完整页面**（`GlobalPage`：**原地接管官方 `conversation` 槽**——`ctx.slots.register({ name:'conversation', priority:-1 })`，ui-slots shadowing 规则「最低 priority 渲染」顶掉 `ui-conversation`（默认 0），chat box 的主区域本身变成全局工作区页（非浮层/非全屏），关闭 dispose 后聊天恢复（会话状态在 store 不受影响）；页面经 `useSyncExternalStore` 订阅 windows store 实时刷新；头部标题/✕/Esc 关闭；经 `sidebar.footer.action` 注入的官方左栏「全局工作区」按钮或 tab 内「展开为完整页面」打开）+ **面板 tab 紧凑视图**（`GlobalView`，+ 菜单可达）。内容用 **SideCard 设置同款「icon 卡片」栅格**（响应式 auto-fill 卡片：图标 chip + 标题 + 共享徽标 + 点亮态，与 DSH 设置页的「icon 设置」UI 同步）。见 `src/client/official-sidebar.tsx` / `src/client/GlobalPage.tsx` / `src/client/GlobalView.tsx` / `docs/plans/2026-08-20-left-sidebar-injection-design.md` |
