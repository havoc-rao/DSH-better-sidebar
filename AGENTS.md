...
| `global` | 60 | 是 | 否 | 全局工作区页：记录所有**实例级全局状态**——目前是全局共享窗口（`gb:` 前缀的「全局共享」终端）清单。**全局共享窗口的完整生命周期（同一个 xterm 会话/PTY）驻留在此**：`bindGlobal`（终端右键「全局共享（转移到全局工作区）」）把终端转入全局 blob（`localStorage: dsh-sidebar:v1:global-windows`），**不再自动合并 stub 到任何 session 的 tab 栏**（旧「所有项目同步」语义改为按需 attach）；会话经 **`attachGlobal`** 把窗口带到自己——`gb:` stub 落进该会话首个叶子并聚焦（attach 到同一个 `shared:gb:<n>` PTY，一处 shell 多处视图），attached stub **持久化在会话布局**（reload 对照全局 blob 校验，窗口已取消则剥离）；attached stub 的 ✕ 只 **detach 本会话**（窗口与共享 PTY 存活）；卡片 ✕ = `unbindGlobal(false)`（全实例关闭 + 释放 PTY）。数据来自宿主注入的 `TabComponentProps.globalWindows`（workspace windows store 的实例级 blob），无则显示引导文案。两种形态：**完整页面**（`GlobalPage`：**原地接管官方 `conversation` 槽**——`ctx.slots.register({ name:'conversation', priority:-1 })`，ui-slots shadowing 规则「最低 priority 渲染」顶掉 `ui-conversation`（默认 0），chat box 的主区域本身变成全局工作区页（非浮层/非全屏），关闭 dispose 后聊天恢复（会话状态在 store 不受影响）；页面经 `useSyncExternalStore` 订阅 windows store 实时刷新；**关闭路径**：Esc，或点击任意会话（页面从无会话 hero 打开，点会话即「打开一个会话」→ 关闭；头部无 ✕ 关闭按钮）；**会话绑定**：页面是**无会话表面**——打开时先记录打开前的 session 再清掉当前 session 的激活态（`openGlobalPage(ctx)` = 捕获 `sessions.list.current` + `ctx.sessions.clear()` + 置 open，页面在 hero 之上打开），因此从页面视角**点任何会话都是「打开一个会话」**，`Sidebar.tsx` 的会话守卫 effect 随即显式关闭页面（规则：仅当「已定义的会话」变为与页面打开时（hero/undefined）不同的会话才关闭；瞬时 `undefined` 重选间隙与回落 hero 不关）。显式关闭同时兜底槽条目重挂崩溃 abdicate 后 open 标志卡死的状态。**页面卡片点击** = 关闭页面 + `attachGlobal` 到打开前的 session + 重新激活该 session（「带着终端回去」）；页面卡片 ✕ = 全实例取消共享。入口：`sidebar.footer.action` 注入的官方左栏「全局工作区」按钮或 tab 内「展开为完整页面」（都走 `openGlobalPage`）+ **面板 tab 紧凑视图**（`GlobalView`，+ 菜单可达）。内容用 **SideCard 设置同款「icon 卡片」栅格**（响应式 auto-fill 卡片：图标 chip + 标题 + 驻留徽标 + 点亮态 + 悬停 ✕，与 DSH 设置页的「icon 设置」UI 同步）。见 `src/client/official-sidebar.tsx` / `src/client/GlobalPage.tsx` / `src/client/GlobalView.tsx` / `src/client/global-page.ts` / `tests/global-page-session.spec.tsx` / `docs/plans/2026-08-20-left-sidebar-injection-design.md` / `docs/plans/2026-08-24-global-workspace-window-parking-design.md` |
...
- **`src/client/official-sidebar.tsx`** / **`src/client/GlobalView.tsx`** / **`src/client/GlobalPage.tsx`** / **`src/client/global-page.ts`**：**注入 DSH 官方左侧栏 + 原地接管主区域**的实现——①脚注按钮：经 `ctx.slots.inject('sidebar.footer.action', …)` 挂进官方 `ui-sidebar` 的加法式脚注槽（`list` 槽，每项只收 `{ wide }`；不替换官方导航列、不改 DSH 源码、HMR-safe）；②完整页面（「全局工作区」）：点击经模块级控制器（`global-page.ts`）触发（打开时先捕获 `sessions.list.current` 再 `ctx.sessions.clear()`——页面是无会话表面，卡片点击据此恢复原会话），插件壳 `Sidebar` 在页面打开时**动态注册进官方 `conversation` 槽（priority -1）**——ui-slots shadowing「最低 priority 渲染」顶掉 `ui-conversation`（默认 0），**chat box 的主区域本身变成全局工作区页**（非浮层/非全屏；`GlobalPage` 经 `useSyncExternalStore` 订阅 windows store 实时刷新；Esc / 打开会话关闭；dispose 后聊天恢复、会话状态在 store 不受影响）；`global` tab（`GlobalView`）是面板内紧凑视图 + 「展开为完整页面」入口。**全局共享窗口的生命周期驻留在此**：卡片点击 = `attachGlobal`（把窗口带到当前/原会话，attach 同一个共享 PTY），卡片 ✕ = `unbindGlobal(false)`（全实例取消共享 + 释放 PTY）；attached `gb:` stub 的 ✕ 只 detach 本会话。页面内容用 **SideCard 设置同款「icon 卡片」栅格**（与 DSH 设置页 UI 同步）。设计见 `docs/plans/2026-08-20-left-sidebar-injection-design.md` / `docs/plans/2026-08-24-global-workspace-window-parking-design.md`，测试 `tests/official-sidebar.spec.tsx`
