...
| `Cmd+Alt+Shift+B` | 进入/退出 IDE 模式（右侧面板占满全屏：宽 100vw、z-index 1000 盖住应用壳、释放布局挤压——类 VSCode 独立窗口；进入自动开面板并把**资源管理器钉在左缘**（无视 `sideBarSide` pref，顺带默认展开 explorer 抽屉）、**侧边对话（Side Chat）钉成右缘聊天列**（`state.chatOpen` 默认展开，主会话被全屏盖住后它就是本模式的对话面；镜像活动 pane 的 sidechat tab，无 tab 时显示 hero，Activity Bar 的 Side Chat 图标是它的开关注，左缘拖拽调宽持久化到 `state.chatWidth`）、**底部面板重新停靠到文件 tabs 区域下方**（z-index 1001，左缘对齐活动栏+文件树列、右缘对齐聊天列左缘），退出恢复停靠宽度；右上角 ✕ 或同键退出） | `!plusMenuOpen` |
