...
| `global` | 60 | 是 | 否 | 全局工作区页：记录所有**实例级全局状态**——目前是全局共享窗口（`gb:` 前缀的「所有项目同步」终端）清单；数据来自宿主注入的 `TabComponentProps.globalWindows`（workspace windows store 的实例级 blob），无则显示引导文案。两种形态：**完整页面**（`GlobalPage`：**原地接管官方 `conversation` 槽**——`ctx.slots.register({ name:'conversation', priority:-1 })`，ui-slots shadowing 规则「最低 priority 渲染」顶掉 `ui-conversation`（默认 0），chat box 的主区域本身变成全局工作区页（非浮层/非全屏），关闭 dispose 后聊天恢复（会话状态在 store 不受影响）；页面经 `useSyncExternalStore` 订阅 windows store 实时刷新；**关闭路径**：Esc，或点击任意会话（页面从无会话 hero 打开，点会话即「打开一个会话」→ 关闭；头部无 ✕ 关闭按钮）；**会话绑定**：页面是**无会话表面**——打开时先清掉当前 session 的激活态（`openGlobalPage(ctx)` = `ctx.sessions.clear()` + 置 open，页面在 hero 之上打开），因此从页面视角**点任何会话都是「打开一个会话」**，`Sidebar.tsx` 的会话守卫 effect 随即显式关闭页面（规则：仅当「已定义的会话」变为与页面打开时（hero/undefined）不同的会话才关闭；瞬时 `undefined` 重选间隙与回落 hero 不关）。显式关闭同时兜底槽条目重挂崩溃 abdicate 后 open 标志卡死的状态。入口：`sidebar.footer.action` 注入的官方左栏「全局工作区」按钮或 tab 内「展开为完整页面」（都走 `openGlobalPage`）+ **面板 tab 紧凑视图**（`GlobalView`，+ 菜单可达）。内容用 **SideCard 设置同款「icon 卡片」栅格**（响应式 auto-fill 卡片：图标 chip + 标题 + 共享徽标 + 点亮态，与 DSH 设置页的「icon 设置」UI 同步）。见 `src/client/official-sidebar.tsx` / `src/client/GlobalPage.tsx` / `src/client/GlobalView.tsx` / `src/client/global-page.ts` / `tests/global-page-session.spec.tsx` / `docs/plans/2026-08-20-left-sidebar-injection-design.md` |
...
- **`src/client/official-sidebar.tsx`** / **`src/client/GlobalView.tsx`** / **`src/client/GlobalPage.tsx`** / **`src/client/global-page.ts`**：**注入 DSH 官方左侧栏 + 原地接管主区域**的实现——①脚注按钮：经 `ctx.slots.inject('sidebar.footer.action', …)` 挂进官方 `ui-sidebar` 的加法式脚注槽（`list` 槽，每项只收 `{ wide }`；不替换官方导航列、不改 DSH 源码、HMR-safe）；②完整页面（「全局工作区」）：点击经模块级控制器（`global-page.ts`）触发，插件壳 `Sidebar` 在页面打开时**动态注册进官方 `conversation` 槽（priority -1）**——ui-slots shadowing「最低 priority 渲染」顶掉 `ui-conversation`（默认 0），**chat box 的主区域本身变成全局工作区页**（非浮层/非全屏；`GlobalPage` 经 `useSyncExternalStore` 订阅 windows store 实时刷新；✕/Esc 关闭；dispose 后聊天恢复、会话状态在 store 不受影响）；`global` tab（`GlobalView`）是面板内紧凑视图 + 「展开为完整页面」入口。页面内容用 **SideCard 设置同款「icon 卡片」栅格**（与 DSH 设置页 UI 同步）。设计见 `docs/plans/2026-08-20-left-sidebar-injection-design.md`，测试 `tests/official-sidebar.spec.tsx`
